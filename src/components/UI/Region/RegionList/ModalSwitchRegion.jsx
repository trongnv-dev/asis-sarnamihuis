...
const ModalSwitchRegion = (props) => {
  const { popUpConfirm, setPopUpConfirm, currentIdRegion, setIsReloadData, isReloadData, isActive } = props;
  const [isLoading, setIsLoading] = useState(false);

  const switchActiveRegion = async (currentIdRegion) => {
    setIsLoading(true);
    try {
      const response = await regionApi.switchActiveRegion({ id: currentIdRegion });
...
export default ModalSwitchRegion;
