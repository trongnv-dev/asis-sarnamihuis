...
const Action = (props) => {
  const { id, handlePopupUpdateLanguage, label } = props;
  ...
  const menuList = [
    <Tooltip title={<ChangeLabel label={label.tooltipUpdate} />} placement="right">
      <div>
        <Button variant="outline-primary" onClick={() => (isSwitch ? null : handlePopupUpdateLanguage(id, isUpdate))}>
    ...
    <Tooltip title={<ChangeLabel label={label.tooltipDetail} />} placement="right">
      <div>
        <Button variant="outline-success" onClick={() => (isSwitch ? null : handlePopupUpdateLanguage(id, !isUpdate))}>
