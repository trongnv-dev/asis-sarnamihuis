...
const ModalSwitchCountry = (props) => {
  const { popUpConfirm, setPopUpConfirm, currentIdCountry, setIsReloadData, isReloadData, isActive } = props;
  const [isLoading, setIsLoading] = useState(false);

  const switchActiveCountry = async (currentIdCountry) => {
    setIsLoading(true);
    try {
      const response = await countryApi.switchActiveCountry({ id: currentIdCountry });
...
export default ModalSwitchCountry;
