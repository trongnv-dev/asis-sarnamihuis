...
const ModalDeleteGender = (props) => {
  const { popUpConfirm, setPopUpConfirm, currentIdGender, setIsReloadData, isReloadData } = props;
  const [isLoading, setIsLoading] = useState(false);

  const switchActiveGender = async (currentIdGender) => {
    setIsLoading(true);
    try {
      const response = await genderApi.deleteItemGender(currentIdGender);
