...
const ModalDeleteSysAttSub = (props) => {
  const { popUpConfirm, setPopUpConfirm, currentIdSysAttSub, setIsReloadData, isReloadData } = props;
  const [isLoading, setIsLoading] = useState(false);

  const switchActiveSysAttSub = async (currentIdSysAttSub) => {
    setIsLoading(true);
    try {
      const response = await sysAttSubApi.deleteItemSysAttSub(currentIdSysAttSub);
...
export default ModalDeleteSysAttSub;
