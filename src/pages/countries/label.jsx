...
import { COUNTRY_LABEL_COLUMN } from 'config/data-table-column';
...
import { Action, UpdateLabelModal } from '../../components/UI/Country/CountryLabel';
import {
  ExportData,
  ImportData,
  SearchByColumn,
...
const codeTable = 'country.label';

const CountryLabelPage = (props) => {
  ...
  const [columns, setColumns] = useState(COUNTRY_LABEL_COLUMN);
  ...
  const handlePopupUpdate = (idLabel, tableName, sort, isUpdate) => {
    if (isUpdate) {
      router.push(`/countries/label/update/${idLabel}`);
    } else {
      router.push(`/countries/label/${idLabel}`);
    }
  };
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const countryLabel = getCountryLabel(data);
              ...
              <Portlet>
                <Portlet.Header>
                  <div className={styles.tableHeader}>
                    <ToggleEditLabel label={countryLabel.tooltipSwitch} />
                    <ImportData label={countryLabel} api={countryApi.importDataLabel} handleReload={handleReload} />
                    <ExportData
                      label={countryLabel}
                      data={data}
                        ...
                        <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size="medium">
                          ...
                          <TableBody>
                            {dataOnPage.map((row, index) => (
                              ...
                              >
                                {columns.map((column) =>
                                  column.visibility && column.key === 'action' ? (
                                    <TableCell
                                      align="left"
                                      width="10px"
...
export default withAuth(withLayout(() => <TableWithConfig CodeTable={codeTable} PageView={CountryLabelPage} />));
